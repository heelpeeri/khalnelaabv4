import type { QuizQuestion } from "./index";

export const geographyQuestions: QuizQuestion[] = [
  {
    question: "ما الدولة الخليجية التي لها حدود برية مع دولة واحدة فقط؟",
    answer: "قطر",
    options: ["الكويت", "قطر", "الإمارات", "عُمان"],
    },
    {
    question: "ما هي الدولة الأفريقية الوحيدة التي تمتلك سواحل على كل من البحر الأبيض المتوسط والمحيط الأطلسي؟",
    answer: "المغرب",
    options: ["مصر", "الجزائر", "الموريتانيا", "المغرب"],
    },
    {
    question: "في أي دولة يقع المقر الرئيسي لمنظمة الأمم المتحدة؟",
    answer: "الولايات المتحدة الأمريكية",
    options: ["سويسرا", "المملكة المتحدة", "الولايات المتحدة الأمريكية", "النمسا"],
    },
    {
    question: "ما هي الدولة الأفريقية التي تُعرف بلقب 'بلاد المليون ونصف المليون شهيد'؟",
    answer: "الجزائر",
    options: ["السودان", "الجزائر", "إثيوبيا", "جنوب أفريقيا"],
    },
    {
    question: "ما هو النهر الذي يمتلك أكبر حوض تصريف في العالم وأكبر حجم لتدفق المياه؟",
    answer: "نهر الأمازون",
    options: ["نهر النيل", "نهر الأمازون", "نهر المسيسيبي", "نهر اليانغتسي"],
    },
    {
    question: "ما اسم المعلم التاريخي الشهير الظاهر في الصورة؟",
    image: "/images/hagia-sophia.jpg",
    answer: "آيا صوفيا",
    options: ["آيا صوفيا", "المسجد الأزرق", "مسجد الفاتح", "قصر طوب قابي"],
    },
    {
    question: "ما هي أصغر دولة في العالم من حيث المساحة؟",
    answer: "الفاتيكان",
    options: ["موناكو", "سان مارينو", "الفاتيكان", "ناورو"],
  },
  {
    question: "ما اسم الدولة المظللة في خريطة قارة أفريقيا؟",
    image: "/images/sudan-in-africa-map.jpg",
    answer: "السودان",
    options: ["مصر", "السودان", "تشاد", "إثيوبيا"],
  },
  {
    question: "ما هو المسطح المائي شديد الملوحة الذي تنعدم فيه الحياة البحرية تقريبًا؟",
    answer: "البحر الميت",
    options: ["البحر الأحمر", "البحر الميت", "بحر قزوين", "البحر الأسود"],
  },
  {
    question: "في أي قارة تقع سلسلة جبال الألب؟",
    answer: "أوروبا",
    options: ["آسيا", "أفريقيا", "أوروبا", "أمريكا الشمالية"],
  },
  {
    question: "ما هي الدولة التي تمتلك أكبر عدد من الأهرامات في العالم؟",
    answer: "السودان",
    options: ["مصر", "المكسيك", "السودان", "بيرو"],
  },
  {
    question: "ما هي أصغر قارة في العالم من حيث المساحة؟",
    answer: "أستراليا",
    options: ["أوروبا", "أنتاركتيكا", "أستراليا", "أمريكا الجنوبية"],
  },
  {
    question: "ما هي الدولة التي تمتلك أطول حدود برية مع الولايات المتحدة الأمريكية؟",
    answer: "كندا",
    options: ["المكسيك", "روسيا", "الصين", "كندا"],
  },
  {
    question: "ما اسم المضيق الظاهر في هذه الخريطة، والذي يفصل بين المغرب وإسبانيا؟",
    image: "/images/gibraltar-strait.jpg",
    answer: "مضيق جبل طارق",
    options: ["مضيق البوسفور", "قناة السويس", "مضيق تيران", "مضيق جبل طارق"],
  },
  {
    question: "ما هو النهر الرئيسي الذي يتدفق عبر جمهورية السودان من الجنوب إلى الشمال؟",
    answer: "النيل",
    options: ["دجلة", "الفرات", "نهر أم الربيع", "النيل"],
  },
  {
    question: "في أي دولة يقع المعلم الطبيعي المسمى صخرة الروشة؟",
    image: "/images/raouche-rock.jpg",
    answer: "لبنان",
    options: ["سوريا", "لبنان", "تونس", "المغرب"],
  },
  {
    question: "ما هما الدولتان العربيتان اللتان يمر بهما نهر الفرات قبل مصبه؟",
    answer: "سوريا والعراق",
    options: ["سوريا والعراق", "الأردن وفلسطين", "مصر والسودان", "لبنان وسوريا"],
  },
  {
    question: "ما هي أكبر دولة في قارة أمريكا الجنوبية من حيث المساحة؟",
    answer: "البرازيل",
    options: ["الأرجنتين", "بيرو", "البرازيل", "كولومبيا"],
  },
  {
    question: "في أي قارة يقع المقر الرئيسي لمنظمة أوبك (OPEC)؟",
    answer: "أوروبا",
    options: ["أوروبا", "آسيا", "أفريقيا", "أمريكا"],
  },
  {
    question: "ما اسم القارة الظاهرة في الصورة؟",
    image: "/images/south-america-map.jpg",
    answer: "أمريكا الجنوبية",
    options: ["أمريكا الجنوبية", "أفريقيا", "آسيا", "أوروبا"],
  },
  {
    question: "كم يبلغ عدد الولايات في الولايات المتحدة الأمريكية؟",
    answer: "50",
    options: ["50", "48", "52", "51"],
  },
  {
    question: "ما هي أكبر صحراء في العالم مساحةً؟",
    answer: "أنتاركتيكا",
    options: ["الصحراء الكبرى", "أنتاركتيكا", "الربع الخالي", "صحراء جوبي"],
  },
  {
    question: "في أي مدينة يقع المعلم التاريخي الظاهر في الصورة؟",
    image: "/images/colosseum.jpg",
    answer: "روما",
    options: ["روما", "ميلان", "فينيسيا", "نابولي"],
  },
  {
    question: "ما هي أطول سلسلة جبال قارية في العالم؟",
    answer: "الأنديز",
    options: ["الأنديز", "الهيمالايا", "الألب", "روكي"],
  },
  {
    question: "ما هي الدولة الأوروبية التي تُعرف جغرافياً بـ 'بلاد الألف بحيرة'؟",
    answer: "فنلندا",
    options: ["فنلندا", "السويد", "النرويج", "كندا"],
  },
  {
    question: "ما هي الدولة التي يمثّلها هذا العلم؟",
    image: "/images/vietnam-flag.jpg",
    answer: "فيتنام",
    options: ["فيتنام", "الصين", "كوريا الشمالية", "تركيا"],
  },
  {
    question: "في أي مدينة أمريكية يقع هذا البرج الشهير؟",
    image: "/images/willis-tower.jpg",
    answer: "شيكاغو",
    options: ["شيكاغو", "نيويورك", "لوس أنجلوس", "هيوستن"],
  },
  {
    question: "في أي ولاية أمريكية يقع الأخدود العظيم (الجراند كانيون)؟",
    image: "/images/grand-canyon.jpg",
    answer: "أريزونا",
    options: ["أريزونا", "كاليفورنيا", "نيفادا", "يوتا"],
  },
  {
    question: "ما هي القارة التي تضم أكبر عدد من الدول المعترف بها؟",
    answer: "أفريقيا",
    options: ["أفريقيا", "آسيا", "أوروبا", "أمريكا الجنوبية"],
  },
  {
    question: "ما اسم الجبل البركاني الشهير الظاهر في الصورة؟",
    image: "/images/mount-fuji.jpg",
    answer: "جبل فوجي",
    options: ["جبل فوجي", "إيفرست", "كي2", "كليمنجارو"],
  },
  {
    question: "ما اسم هذا المعلم التاريخي الشهير؟",
    image: "/images/taj-mahal.jpg",
    answer: "تاج محل",
    options: ["تاج محل", "مسجد الشيخ زايد", "آيا صوفيا", "ضريح همايون"],
  },
  {
    question: "ما اسم المدينة الظاهرة في هذه اللقطة الجوية؟",
    image: "/images/new-york-skyline.jpg",
    answer: "نيويورك",
    options: ["نيويورك", "شيكاغو", "تورونتو", "بوسطن"],
  },
  {
    question: "ما اسم النصب التذكاري المنحوت على الجبل والظاهر في الصورة؟",
    image: "/images/mount-rushmore.jpg",
    answer: "جبل راشمور",
    options: ["جبل راشمور", "تماثيل جزيرة الفصح", "نقوش لونغمِن الصخرية", "تماثيل باميان"],
  },
  {
    question: "ما هي الدولة العربية التي تحد المملكة الأردنية الهاشمية من جهة الغرب؟",
    answer: "فلسطين",
    options: ["فلسطين", "سوريا", "العراق", "السعودية"],
  },
  {
    question: "ما هي الدولة التي يمثّلها هذا العلم؟",
    image: "/images/indonesia-flag.jpg",
    answer: "إندونيسيا",
    options: ["إندونيسيا", "بولندا", "موناكو", "النمسا"],
  },
  {
    question: "ما هي الدولة التي يمثّلها هذا العلم؟",
    image: "/images/belgium-flag.jpg",
    answer: "بلجيكا",
    options: ["بلجيكا", "فرنسا", "ألمانيا", "رومانيا"],
  },
  {
    question: "في أي دولة عربية يقع المعلم الأثري الظاهر في الصورة؟",
    image: "/images/petra-monastery.jpg",
    answer: "الأردن",
    options: ["الأردن", "لبنان", "مصر", "المغرب"],
  },
  {
    question: "ما اسم الممر المائي الاستراتيجي الظاهر في الخريطة؟",
    image: "/images/bab-al-mandab.jpg",
    answer: "باب المندب",
    options: ["باب المندب", "مضيق هرمز", "قناة السويس", "مضيق جبل طارق"],
  },
  {
    question: "ما الدولة التي تقع أراضيها بين قارتي آسيا وأوروبا وتضم الأناضول وتراقيا الشرقية؟",
    answer: "تركيا",
    options: ["تركيا", "قبرص", "أرمينيا", "جورجيا"],
  },
  {
    question: "ما هي أكبر مدينة في أستراليا من حيث عدد السكان، والتي غالباً ما يُعتقد خطأً أنها العاصمة؟",
    answer: "سيدني",
    options: ["سيدني", "أوتاوا", "أوسلو", "برن"],
  },
  {
    question: "ما هي العاصمة السياسية والرسمية لدولة أستراليا؟",
    answer: "كانبيرا",
    options: ["سيدني", "ملبورن", "كانبيرا", "بيرث"],
  },
  {
    question: "ما هي الدولة الآسيوية التي تمتلك حدوداً برية مع 14 دولة؟",
    answer: "الصين",
    options: ["الصين", "روسيا", "ألمانيا", "البرازيل"],
  },
  {
    question: "ما هو أطول نهر في العالم؟",
    answer: "النيل",
    options: ["الأمازون", "النيل", "اليانغتسي", "المسيسيبي"],
  },
  {
    question: "ما هو أكبر محيط في العالم من حيث المساحة؟",
    answer: "الهادئ",
    options: ["الهادئ", "الأطلسي", "الهندي", "المتجمد الشمالي"],
  },
  {
    question: "ما هي أكبر دولة في قارة أفريقيا من حيث المساحة؟",
    answer: "الجزائر",
    options: ["الجزائر", "مصر", "نيجيريا", "السودان"],
  },
  {
    question: "في أي قارة تقع دولة البرازيل؟",
    answer: "أمريكا الجنوبية",
    options: ["أمريكا الجنوبية", "أوروبا", "آسيا", "أفريقيا"],
  },
  {
    question: "ما هي عاصمة دولة منغوليا؟",
    answer: "أولان باتور",
    options: ["أولان باتور", "بكين", "طشقند", "أستانا"],
  },
  {
    question: "مدينة برن هي عاصمة لأي دولة أوروبية؟",
    answer: "سويسرا",
    options: ["سويسرا", "النمسا", "ألمانيا", "فرنسا"],
  },
  {
    question: "مدينة وارسو هي عاصمة لأي دولة أوروبية؟",
    answer: "بولندا",
    options: ["بولندا", "هنغاريا", "التشيك", "النمسا"],
  },
  {
    question: "مدينة ليما هي عاصمة لأي دولة في أمريكا الجنوبية؟",
    answer: "بيرو",
    options: ["بيرو", "تشيلي", "الأرجنتين", "كولومبيا"],
  },
];
