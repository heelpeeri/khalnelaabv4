...
function GameLoading() {
  return (
    <div className="arcade-card mx-auto w-full max-w-5xl p-8 text-center">
      <p className="text-sm font-black text-cyan-200/70">جاري تجهيز اللعبة...</p>
    </div>
  );
}

const WordGame = dynamic(GAME_LOADERS.word, { ssr: false, loading: GameLoading });
const QuizGame = dynamic(GAME_LOADERS.quiz, { ssr: false, loading: GameLoading });
const ScrambleGame = dynamic(GAME_LOADERS.scramble, { ssr: false, loading: GameLoading });
const WheelGame = dynamic(GAME_LOADERS.wheel, { ssr: false, loading: GameLoading });
const CategoriesGame = dynamic(GAME_LOADERS.categories, { ssr: false, loading: GameLoading });
const ProverbGame = dynamic(GAME_LOADERS.draw, { ssr: false, loading: GameLoading });
...
function getGameStats(roundStats: RoundStat[]) {
  return roundStats.reduce<GameStat[]>((result, stat) => {
    const existing = result.find((item) => item.game === stat.game);

    if (existing) {
      existing.total += 1;
      if (stat.winner === "side1") existing.side1Wins += 1;
      if (stat.winner === "side2") existing.side2Wins += 1;
      if (stat.winner === "none") existing.draws += 1;
      return result;
    }
...
function WinnerOverlay({
  ...
  const totalDraws = roundStats.filter((stat) => stat.winner === "none").length;
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/75 px-4 py-4 backdrop-blur-md">
      <div className="arcade-card w-full max-w-5xl p-5 text-center animate-fade-in-up sm:p-7">
        ...
        <h1 className="arcade-title mt-3">{isDraw ? "تعادل! 🤝" : "كفووو! 🏆"}</h1>
        ...
        <div className="mx-auto mt-6 grid max-w-3xl grid-cols-[1fr_auto_1fr] items-stretch gap-4">
          ...
          <div className="flex items-center justify-center text-3xl font-black text-white/25">-</div>
          ...
          <div className="mt-6 border-t border-white/10 pt-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div className="text-right">
                <p className="text-xl font-black text-white">إحصائيات الجلسة</p>
                <p className="text-sm font-bold text-white/40">نتائج الألعاب</p>
              </div>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {gameStats.map((stat) => {
                const side1Won = stat.side1Wins > stat.side2Wins;
                const side2Won = stat.side2Wins > stat.side1Wins;
                      ...
                      <div className="shrink-0">
                        ...
                        {stat.draws > 0 && (
                          <p className="mt-2 text-center text-xs font-bold text-white/45">
                            {stat.draws === 1 ? "تعادل واحد" : `${stat.draws} تعادل`}
                          </p>
                        )}
                      </div>

                      <div className="min-w-0 text-right">
                        <p className="truncate text-lg font-black text-white">{getGameName(stat.game)}</p>
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <button type="button" onClick={onRestart} className="arcade-button">
  ...
  return (
    <div className="fixed inset-0 z-[998] flex items-center justify-center bg-black/80 px-4 backdrop-blur-md">
      <div className="arcade-card w-full max-w-xl p-8 text-center animate-fade-in-up">
        ...
        <h1 className="arcade-title mt-6">{countdown === 0 ? "يلا!" : countdown}</h1>
      </div>
    </div>
  );
}
...
function TransitionOverlay({ nextRound }: { nextRound: Round }) {
  return (
    <div className="fixed inset-0 z-[998] flex items-center justify-center bg-black/80 px-4 backdrop-blur-md">
      <div className="arcade-card w-full max-w-xl p-8 text-center animate-fade-in-up">
        <p className="text-sm font-black tracking-[0.22em] text-cyan-300/80">الجولة الجاية</p>
        <h1 className="mt-6 text-4xl font-black text-white">{getGameName(nextRound.game)}</h1>
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/70 px-4 backdrop-blur-md">
      <div className="arcade-card w-full max-w-2xl p-8 text-center animate-fade-in-up">
        <p className="text-sm font-black tracking-[0.22em] text-cyan-300/80">نهاية الجولة</p>
        <h1 className="arcade-title mt-5">من فاز؟ 🏆</h1>
        <p className="mt-3 text-lg font-bold text-white/70">اختر الفائز في هذه الجولة</p>

        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          <button type="button" onClick={() => onPick("side1")} className="arcade-button px-6 py-4 text-lg">
            {side1Name || "فريق 1"}
          </button>

          <button type="button" onClick={() => onPick("side2")} className="arcade-button px-6 py-4 text-lg">
            {side2Name || "فريق 2"}
          </button>
        </div>

        <button type="button" onClick={() => onPick("none")} className="btn-secondary mt-4 w-full px-6 py-4 text-lg">
          لا أحد
        </button>
      </div>
    </div>
...
function RoundResultOverlay({
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/70 px-4 backdrop-blur-md">
      <div className="arcade-card w-full max-w-2xl p-8 text-center animate-fade-in-up">
        <p className="text-sm font-black tracking-[0.22em] text-cyan-300/80">انتهت الجولة</p>
        <h1 className="arcade-title mt-5">{isDraw ? "تعادل! 🤝" : "كفووو! 🏆"}</h1>
        <p className="arcade-winner mt-6">
          {isDraw ? "محد أخذ نقطة الجولة" : `فاز بالجولة: ${winnerName}`}
        </p>
      </div>
    </div>
  );
...
async function createResultShareImage({
  ...
  background.addColorStop(0, "#06020e");
  background.addColorStop(0.5, "#170628");
  background.addColorStop(1, "#050914");
  ctx.fillStyle = background;
  ctx.fillRect(0, 0, width, height);

  // Glows
  const pinkGlow = ctx.createRadialGradient(150, 470, 20, 150, 470, 430);
  pinkGlow.addColorStop(0, "rgba(217,70,239,0.30)");
  pinkGlow.addColorStop(1, "rgba(217,70,239,0)");
  ctx.fillStyle = pinkGlow;
  ctx.fillRect(0, 0, width, height);

  const cyanGlow = ctx.createRadialGradient(930, 500, 20, 930, 500, 430);
  cyanGlow.addColorStop(0, "rgba(34,211,238,0.25)");
  cyanGlow.addColorStop(1, "rgba(34,211,238,0)");
  ctx.fillStyle = cyanGlow;
  ctx.fillRect(0, 0, width, height);

  // Main card
  drawRoundedRect(ctx, 65, 60, 950, 1230, 48);
  const cardGradient = ctx.createLinearGradient(65, 60, 1015, 1290);
  cardGradient.addColorStop(0, "rgba(33,8,57,0.96)");
  cardGradient.addColorStop(1, "rgba(9,6,27,0.98)");
  ...
  ctx.fillText("خل نلعب 🎮", width / 2, 135);

  // Main result
  ctx.fillStyle = "#ffffff";
  ctx.font = "900 72px Arial";
  ctx.fillText(isDraw ? "تعادل! 🤝" : "كفووو! 🏆", width / 2, 245);

  ctx.font = "900 40px Arial";
  ctx.fillStyle = "#f5e9ff";
  ctx.fillText(
    isDraw ? "الفريقين قدّها" : `الفائز: ${finalWinnerName}`,
    width / 2,
    315
  );

  // Team 1
  drawRoundedRect(ctx, 105, 390, 390, 270, 38);
  const team1Gradient = ctx.createLinearGradient(105, 390, 495, 660);
  team1Gradient.addColorStop(0, "rgba(217,70,239,0.55)");
  team1Gradient.addColorStop(1, "rgba(126,34,206,0.25)");
  ctx.fillStyle = team1Gradient;
  ctx.fill();
  ctx.strokeStyle = "rgba(240,171,252,0.75)";
  ctx.lineWidth = 3;
  ctx.stroke();

  ctx.fillStyle = "#fae8ff";
  ctx.font = "900 32px Arial";
  ctx.fillText(side1 || "فريق 1", 300, 460);
  ctx.font = "900 110px Arial";
  ctx.fillText(String(side1Score), 300, 590);

  // Team 2
  drawRoundedRect(ctx, 585, 390, 390, 270, 38);
  const team2Gradient = ctx.createLinearGradient(585, 390, 975, 660);
  team2Gradient.addColorStop(0, "rgba(34,211,238,0.52)");
  team2Gradient.addColorStop(1, "rgba(37,99,235,0.25)");
  ctx.fillStyle = team2Gradient;
  ctx.fill();
  ...
  ctx.font = "900 32px Arial";
  ctx.fillText(side2 || "فريق 2", 780, 460);
  ctx.font = "900 110px Arial";
  ctx.fillText(String(side2Score), 780, 590);
  ...
  // Session details
  const draws = roundStats.filter((stat) => stat.winner === "none").length;
  const roundText = getRoundsLabel(roundStats.length);
  const drawText =
    draws === 0 ? "بدون تعادل" : draws === 1 ? "تعادل واحد" : `${draws} تعادل`;

  ctx.fillStyle = "#ffffff";
  ctx.font = "900 40px Arial";
  ctx.fillText("نتيجة الجلسة", width / 2, 755);

  ctx.fillStyle = "rgba(255,255,255,0.68)";
  ctx.font = "700 28px Arial";
  ctx.fillText(`${roundText}  •  ${drawText}`, width / 2, 810);

  // CTA card
  drawRoundedRect(ctx, 145, 895, 790, 205, 38);
  ...
  ctx.fillText("تقدرون تتحدونا؟ 👀", width / 2, 975);

  ctx.fillStyle = "rgba(255,255,255,0.60)";
  ctx.font = "700 27px Arial";
  ctx.fillText("العبوا خل نلعب وشوفوا مين يفوز", width / 2, 1035);

  // Footer
  ctx.fillStyle = "#67e8f9";
  ctx.font = "900 38px Arial";
  ctx.fillText("خل نلعب", width / 2, 1180);

  ctx.fillStyle = "rgba(255,255,255,0.38)";
  ctx.font = "600 23px Arial";
  ctx.fillText(window.location.host, width / 2, 1230);

  const blob = await new Promise<Blob | null>((resolve) => {
    canvas.toBlob(resolve, "image/png", 1);
  });

  if (!blob) throw new Error("فشل إنشاء صورة النتيجة");

  return new File([blob], "khal-nelab-result.png", {
    type: "image/png",
  });
}
...
export default function MatchPage() {
  ...
  const [roundResultWinner, setRoundResultWinner] = useState<WinnerType>("none");
  ...
  function clearFlowTimers() {
    ...
    if (roundResultTimer.current) {
      clearTimeout(roundResultTimer.current);
      roundResultTimer.current = null;
    }
  }
  ...
  function start() {
    ...
    const builtQueue = buildRoundQueue(selectedGames, gameRounds, quizCategories);

    // نبدأ تحميل الألعاب المختارة أثناء العد التنازلي،
    // عشان غالبًا تكون جاهزة قبل ما تبدأ الجولة.
    preloadGames(selectedGames);
  ...
  function goToNextRound(nextSide1Score: number, nextSide2Score: number) {
    ...
    const upcomingRound = queue[index + 1];
    if (upcomingRound) preloadGames([upcomingRound.game]);
  ...
  function applyRoundWinner(winner?: WinnerType) {
    ...
    const finalWinner = winner ?? "none";
    const nextSide1Score = side1Score + (finalWinner === "side1" ? 1 : 0);
    const nextSide2Score = side2Score + (finalWinner === "side2" ? 1 : 0);
    ...
    setRoundResultWinner(finalWinner);
    setShowRoundResult(true);

    roundResultTimer.current = setTimeout(() => {
      setShowRoundResult(false);
      roundResultTimer.current = null;
  ...
  useEffect(() => {
    ...
    if (urlMode === "session" || !isGameType(gameParam)) {
      setMode("session");
      return;
    }
    ...
    setGameRounds({ [gameParam]: getDefaultRounds(gameParam) });
    setQuizCategories(gameParam === "quiz" && category ? [category] : []);

    // حمّل اللعبة المختارة بالخلفية من لحظة فتح صفحة الإعداد.
    preloadGames([gameParam]);
  }, []);
  ...
  const roundWinnerName =
    roundResultWinner === "side1"
      ...
      : roundResultWinner === "side2"
        ? side2 || "فريق 2"
        : "";
  const roundIsDraw = roundResultWinner === "none";

  async function handleShareResult() {
    try {
      const imageFile = await createResultShareImage({
        isDraw,
        finalWinnerName,
        side1,
        side2,
      ...
      link.href = imageUrl;
      link.download = "khal-nelab-result.png";
  ...
  function renderCurrentGame() {
    ...
    switch (current.game) {
      ...
      case "quiz":
        return <QuizGame {...commonGameProps} category={current.category} />;

      case "scramble":
        return <ScrambleGame {...commonGameProps} />;

      case "wheel":
        return <WheelGame {...commonGameProps} />;

      case "categories":
        return <CategoriesGame {...commonGameProps} />;

      case "draw":
        return <ProverbGame {...commonGameProps} />;
    }
  }
  ...
  return (
    <main className="min-h-screen p-6 text-white">
      ...
      {phase === "transition" && nextRound && <TransitionOverlay nextRound={nextRound} />}
