...
export default function Home() {
  ...
  // حمّل صفحة اللعب بالخلفية بعد ما تفتح الرئيسية،
  // عشان الانتقال لأول لعبة يكون أسرع.
  useEffect(() => {
      ...
      <div className="mx-auto max-w-[1450px]">
        ...
        <section className="relative rounded-[36px] border border-purple-400/20 bg-[#0c0617]/75 px-5 pb-4 pt-5 shadow-[0_30px_100px_rgba(0,0,0,0.36)] backdrop-blur-2xl sm:px-8 lg:px-10">
          ...
          <Link
            href="/guide"
            prefetch
            className="absolute left-5 top-5 z-50 flex items-center gap-2 rounded-full border border-cyan-300/30 bg-[#130b24]/90 px-4 py-2.5 text-sm font-black text-white shadow-[0_0_20px_rgba(34,211,238,0.12)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-0.5 hover:border-cyan-300/55 hover:bg-[#19102c] hover:shadow-[0_0_26px_rgba(34,211,238,0.20)]"
          >
            <span>🎮</span>
          ...
          <div className="text-center">
            <h1 className="text-4xl font-black sm:text-5xl">🏆 تحدي الجلسة</h1>
            ...
            <div className="mx-auto mt-2 max-w-2xl rounded-2xl border border-yellow-300/[0.08] bg-gradient-to-r from-yellow-300/[0.035] via-yellow-200/[0.06] to-yellow-300/[0.035] px-5 py-2">
              <p className="text-sm font-bold leading-6 text-yellow-50/65 sm:text-[15px]">
                اختاروا أكثر من لعبة، سجلوا الفرق مرة وحدة، واجمعوا النقاط لين يطلع بطل الجلسة.
              </p>
            </div>
            ...
            <Link
              ...
              prefetch
              className="mt-3 inline-flex items-center justify-center rounded-full border border-yellow-200/45 bg-gradient-to-r from-yellow-300 to-amber-400 px-10 py-3 font-black text-[#241600] shadow-[0_10px_32px_rgba(250,204,21,0.24)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_16px_42px_rgba(250,204,21,0.35)]"
            >
              ابدأ تحدي الجلسة
            </Link>
          </div>
          ...
          <div className="relative mx-auto mt-3 max-w-[1120px] rounded-[28px] border border-cyan-300/[0.14] bg-gradient-to-b from-cyan-400/[0.045] via-white/[0.02] to-transparent px-3 pb-1 pt-3 shadow-[inset_0_1px_0_rgba(255,255,255,.025)] sm:px-5">
            ...
            <div className="relative z-10 text-center">
              <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/25 bg-cyan-400/[0.09] px-5 py-1.5 shadow-[0_0_18px_rgba(34,211,238,.08)]">
                ...
                <span className="text-sm font-black text-cyan-100">لعبة سريعة</span>
              </div>

              <h2 className="mt-1.5 text-2xl font-black sm:text-[28px]">اختر لعبة وابدأ مباشرة</h2>
              <p className="mt-0.5 text-xs font-bold text-white/40">اضغط على أي بطاقة</p>
            </div>
            ...
            >
              ...
              <div className="absolute left-1/2 top-[49%]">
                {GAME_CARDS.map((card, index) => {
                  ...
                  const spread =
                    hoveredCard === null ? 0 : index < hoveredCard ? -28 : index > hoveredCard ? 28 : 0;
                  const finalX = card.x + spread;
                  const finalY = active ? card.y - 34 : card.y;
                  ...
                  return (
                    <Link
                      ...
                      className="absolute left-1/2 top-1/2 cursor-pointer outline-none will-change-transform"
                      style={{
                        width: "185px",
                        height: "264px",
                        zIndex: active ? 100 : card.z,
                        transform: `translate(-50%, -50%) translate3d(${finalX}px, ${finalY}px, 0) scale(${active ? card.scale + 0.11 : card.scale})`,
                        transition: CARD_TRANSITION,
                        filter: active
                          ? "drop-shadow(0 26px 26px rgba(0,0,0,.55)) drop-shadow(0 0 28px rgba(168,85,247,.34))"
                          : "drop-shadow(0 14px 16px rgba(0,0,0,.30))",
                        opacity: hoveredCard !== null && !active ? 0.76 : 1,
                      }}
                    >
                      <div
                        className={`game-idle-card relative h-full w-full overflow-hidden rounded-[21px] border bg-[#11091f] ${
                          active
                      ...
                      >
                        ...
                        <div
                          className={`pointer-events-none absolute inset-x-3 bottom-3 rounded-full border border-white/10 bg-black/60 px-3 py-2 text-center text-xs font-black backdrop-blur-md transition-all duration-500 ${
            ...
            <div className="relative z-10 mt-4 grid grid-cols-2 gap-3 pb-2 sm:grid-cols-3 lg:hidden">
              {GAME_CARDS.map((card) => (
                <Link key={card.title} href={card.href} prefetch className="group">
                  <div
                    className="game-mobile-card relative aspect-[3/4] overflow-hidden rounded-[20px] border border-white/10 shadow-[0_12px_30px_rgba(0,0,0,.30)] transition-all duration-300 group-active:scale-[0.97]"
                    style={{ animationDelay: `${card.delay}s` }}
                  >
                    <Image
                      src={card.image}
