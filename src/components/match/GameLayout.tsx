...
export default function GameLayout({
  ...
  const isSide1Turn =
    ...
    cleanTurn.includes('فريق 1');

  const isSide2Turn =
    cleanTurn === cleanSide2 ||
    cleanTurn.includes(cleanSide2) ||
    cleanTurn.includes('فريق 2');
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-3 sm:px-4 lg:px-6">
      <div className="glass rounded-[24px] border border-white/10 bg-[#121028]/80 p-3 text-center shadow-[0_0_30px_rgba(0,0,0,0.35)] backdrop-blur-xl sm:rounded-[28px] sm:p-4 lg:p-6">
        ...
        {/* Teams and current turn */}
        <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-3">

          {/* Team 1 */}
          <div
            className={`flex min-h-[128px] flex-col items-center justify-center rounded-2xl border px-4 py-4 transition-all duration-300 ${
              isSide1Turn
                ? 'scale-[1.02] border-fuchsia-300/70 bg-gradient-to-br from-fuchsia-500/30 to-pink-500/15 shadow-[0_0_30px_rgba(217,70,239,0.28)]'
                : 'border-fuchsia-300/15 bg-fuchsia-500/[0.07] opacity-65'
            }`}
          >
            <p
              className={`text-sm font-bold ${
                isSide1Turn ? 'text-fuchsia-100' : 'text-white/50'
              }`}
            >
              {side1}
            </p>
            ...
            <p
              className={`mt-2 flex min-h-[52px] items-center justify-center text-4xl font-black leading-none sm:text-5xl ${
                isSide1Turn ? 'text-fuchsia-100' : 'text-fuchsia-200/65'
              }`}
            >
              {side1Score}
            </p>
          ...
          <div
            className={`flex min-h-[128px] flex-col items-center justify-center rounded-2xl border px-4 py-4 transition-all duration-500 ${
              isSide1Turn
                ? 'border-fuchsia-300/80 bg-gradient-to-br from-fuchsia-500/35 via-pink-500/25 to-purple-500/20 shadow-[0_0_35px_rgba(217,70,239,0.35)]'
                : isSide2Turn
                  ? 'border-cyan-300/80 bg-gradient-to-br from-cyan-400/35 via-sky-500/25 to-blue-500/20 shadow-[0_0_35px_rgba(34,211,238,0.35)]'
          ...
          <div
            className={`flex min-h-[128px] flex-col items-center justify-center rounded-2xl border px-4 py-4 transition-all duration-300 ${
              isSide2Turn
                ? 'scale-[1.02] border-cyan-300/70 bg-gradient-to-br from-cyan-400/30 to-blue-500/15 shadow-[0_0_30px_rgba(34,211,238,0.28)]'
                : 'border-cyan-300/15 bg-cyan-400/[0.07] opacity-65'
            }`}
          >
            <p
              className={`text-sm font-bold ${
                isSide2Turn ? 'text-cyan-100' : 'text-white/50'
              }`}
            >
              {side2}
            </p>
            ...
            <p
              className={`mt-2 flex min-h-[52px] items-center justify-center text-4xl font-black leading-none sm:text-5xl ${
                isSide2Turn ? 'text-cyan-100' : 'text-cyan-200/65'
              }`}
            >
              {side2Score}
            </p>
